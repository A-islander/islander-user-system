...
func resList(list interface{}, count int) map[string]interface{} {
	...
	data["list"] = mapListKeyReset(structList2map(list))
	data["count"] = count
	res := make(map[string]interface{})
	res["code"] = http.StatusOK
	res["data"] = mapKeyReset(data)
	res["msg"] = "ok"
	return res
}
...
func mapKeyReset(data map[string]interface{}) map[string]interface{} {
	...
	for k, v := range data {
		if []byte(k)[0] < 97 {
			newKey := []byte(k)
			newKey[0] += 32
			data[string(newKey)] = v
			delete(data, k)
		}
		if typeof(v) == "map[string]interface {}" {
			mapKeyReset(v.(map[string]interface{}))
		}
	}
end:
	return data
...
func mapListKeyReset(data []map[string]interface{}) []map[string]interface{} {
	for i := 0; i < len(data); i++ {
		data[i] = mapKeyReset(data[i])
	}
	return data
}
